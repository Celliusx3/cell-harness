...
def telegram_channel(gateway: ChannelGateway) -> tuple[TelegramChannel, FakeBot]:
    ...
    never dials api.telegram.org with a made-up token.
    """
    channel = TelegramChannel("123:fake-token", gateway)
    bot = FakeBot()
