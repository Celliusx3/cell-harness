...
export function useConversation(conversationId: string): Conversation {
  ...
  const send = useCallback(
    async (prompt: string) => {
      ...
      try {
        ...
        setRunning(true);
        // The new turn's events arrive on the stream the effect above is already
        // holding open — nothing is appended optimistically here, so the screen
        // can only ever show what the log actually says.
      } catch (err) {
  ...
  return { events, title, running, loading, error, send, stop };
}
