...
5. **Serial vs parallel tool dispatch.** Ship serial. `concurrency_safe` has no
   reason to exist until the fs seam (phase 11) makes overlap meaningful and
   testable — and phase 11 is optional.
6. **How much of the run/SSE layer survives a multi-process future.** cell-bot's
   heartbeat + reclaim already anticipates it; `RunStore` stays a Protocol.
7. **Whether the optional track (11–13) is ever built.** Decide before phase 11,
