...
Loaded automatically into every agent session in this directory (`CLAUDE.md` is a
symlink to this file). Keep it thin — long-form rules and rationale live in
[docs/](./docs/).
...
**Status: phase 1 — "it answers" — implemented, awaiting review.** Next is
...
`Events` is written in phase 2 because the tool pipeline is its first consumer;
`Scope` in phase 4; `Layered` in phase 13. A registry is a flat dict until a
plugin needs to register into one agent's world. Building any of them earlier is
the speculative structure the KISS/YAGNI rule below forbids.
...
| [PHASES.md](./PHASES.md) | 17 phases in 4 milestones — deliverables, contracts, acceptance criteria |
...
| [docs/without-cordis.md](./docs/without-cordis.md) | Scope / Layered / Events in ~250 lines, and what we give up |
